...
const ProgramItinerary = ({
  ...
  return <section id="itinerary" className="py-16 bg-dragon-beige">
      <div className="container-wide">
        ...
        {safeWeeks.length > 0 ? <Tabs defaultValue={`week${safeWeeks[0]?.number || 1}`} className="w-full">
            <TabsList className="grid grid-cols-3 w-full max-w-2xl mx-auto mb-12 h-auto p-2 bg-white/50 backdrop-blur-sm border border-dragon/10">
              {safeWeeks.map(week => <TabsTrigger key={week.number} value={`week${week.number}`} className="flex items-center justify-center py-4 px-3 text-center data-[state=active]:bg-dragon data-[state=active]:text-white data-[state=active]:shadow-lg transition-all duration-200 rounded-lg">
                  <span className="font-bold text-sm">Week {week.number}</span>
                </TabsTrigger>)}
            </TabsList>

            {safeWeeks.map(week => <TabsContent key={week.number} value={`week${week.number}`} className="mt-0">
                <Card className="border-none shadow-md overflow-hidden">
                  <div className="grid md:grid-cols-2">
                    <div className="h-80 md:h-auto overflow-hidden">
                      <img src={week.number === 1 ? "https://res.cloudinary.com/dng12bd0a/image/upload/v1749400855/KleinNettoh--8_1_gtxyvk.jpg" : week.image} alt={`Week ${week.number}: ${week.title}`} className="w-full h-full object-cover" />
                    </div>
                    <CardContent className="p-6 md:p-8">
                      ...
                      <div className="mt-6">
                        ...
                        <ul className="space-y-2">
                          {week.highlights.map((highlight, index) => <li key={index} className="flex items-center">
                      ...
                      <div className="mt-6">
                        <Button variant="outline" className="w-full flex justify-between items-center border-dragon text-dragon" asChild>
          ...
          </Tabs> : <div className="text-center bg-white p-8 rounded-lg shadow-sm">
