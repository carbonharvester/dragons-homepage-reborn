      ...
      <TabsList className="flex flex-wrap justify-center gap-4 mb-10 bg-transparent h-auto">
        {programCategories.map(category => (
          <TabsTrigger 
            key={category.id} 
            value={category.id} 
            className="px-6 py-3 min-w-[160px] h-12 data-[state=active]:bg-dragon data-[state=active]:text-white bg-white border border-dragon text-dragon hover:bg-dragon hover:text-white"
          >
            {category.label}
          </TabsTrigger>
        ))}
